    ...
    G: FnMut() -> anyhow::Result<Box<dyn Memory>>,
{
    match classify_memory_backend(backend_name) {
        ...
        MemoryBackendKind::Hybrid => {
            anyhow::bail!(
                "Hybrid memory backend is not supported in this context{unknown_context}. Use sqlite, lucid, or markdown instead."
            )
        }
...
pub fn create_memory_with_storage_and_routes(
    ...
    if config.snapshot_enabled
        ...
        && matches!(
            backend_kind,
            MemoryBackendKind::Sqlite | MemoryBackendKind::Hybrid | MemoryBackendKind::Lucid
        )
    {
    ...
    // Hybrid backend: SQLite (facts) + Qdrant (vectors)
    if matches!(backend_kind, MemoryBackendKind::Hybrid) {
        let url: String = config
            .qdrant
            .url
            .clone()
            ...
            .filter(|s: &String| !s.trim().is_empty())
            .context(
                "Hybrid memory backend requires url in [memory.qdrant] or QDRANT_URL env var",
            )?;
        let collection = std::env::var("QDRANT_COLLECTION")
            .ok()
            .filter(|s| !s.trim().is_empty())
        ...
        let qdrant = QdrantMemory::new_lazy(&url, &collection, qdrant_api_key, embedder);

        tracing::info!(
            "🔗 Hybrid memory backend configured (sqlite + qdrant: {}, collection: {})",
            url,
            collection
        );

        return Ok(Box::new(HybridMemory::new(sqlite, qdrant)));
    }
