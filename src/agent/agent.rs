...
impl Agent {
    ...
    /// Streaming turn for OpenAI compatibility layer.
    ///
    /// MVP Implementation: Uses existing turn() method and converts
    /// the result to a stream format. This provides full tool support
    /// through the existing Agent::turn() logic.
    ///
    /// For true streaming (SSE from provider), requires Arc<Mutex<Agent>>
    /// refactoring to allow async access to &mut self across await points.
    pub fn turn_streaming(
        ...
        // For MVP, we can't do async work in non-async fn.
        ...
        // Return a stream with informational chunks
        // The actual processing happens in the handler
        let chunks = vec![
    ...
    // ===== Helper methods for streaming (used by turn_streaming in Task 1.5) =====
    ...
    /// Enrich messages with memory recall (RAG).
    /// Placeholder for now - full RAG integration later.
    async fn enrich_with_memory_for_streaming(&self, messages: &[ChatMessage]) -> Vec<ChatMessage> {
        // For now, return messages unchanged
        // Full RAG integration in future enhancement
        messages.to_vec()
    }
    ...
    /// Placeholder for now - full implementation in tool execution task.
    fn build_messages_for_streaming(
        &self,
        base_messages: &[ChatMessage],
        _system_prompt: Option<&str>,
    ) -> Vec<ChatMessage> {
        // For now, return base messages
